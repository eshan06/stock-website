...
def home():
	...
	user1 = request.form.get("name") or "KEY1"

	if user1=="KEY1":
	
		return render_template("navbar2.html", user="No Ticker", charts="", charts2 = "", charts3 = "", balance = "", analyst1 = "", analyst2 = "", analyst3 = "", analyst4 = "", analyst5 = "", he1 = "", he2 = "", he3 = "")
	
	try:
		web.DataReader(user1, 'yahoo')
	except:
		return render_template("navbar2.html", user="Invalid Ticker", charts="", charts2 = "", charts3 = "", balance = "", analyst1 = "", analyst2 = "", analyst3 = "", analyst4 = "", analyst5 = "", he1 = "", he2 = "", he3 = "")
	...
	fig.write_html("dgidash.txt")

	h = open("dgidash.txt", "r")

	start = dt.datetime(2000,1,1)
	end = date.today()

	df = web.DataReader(user1, 'yahoo', start, end)

	data2 = df[['Open', 'High', 'Close']]

	df = df.tail(n=10)

	#data2 = web.DataReader(user1, 'yahoo', start, end)[['Open', 'High', 'Close']]

	hundredma = web.DataReader(user1, 'yahoo', start, end)
	hundredma['100ma']=hundredma['Adj Close'].rolling(window=100).mean()
	hundredma.dropna(inplace=True)
	hundredma.tail(50)
	hundredma[['100ma', 'Close']].plot().write_html("chart2.txt")

	n = data2.plot()

	n.write_html("chart1.txt")

	f = open("chart1.txt", "r")

	balance_sheet = ys.get_balance_sheet(user1)
	g = open("chart2.txt", "r")
	...
	lol = ys.get_analysts_info(user1)
	analyst1=  pd.DataFrame.from_dict(lol['Earnings Estimate'])
	analyst2=  pd.DataFrame.from_dict(lol['Earnings History'])
	analyst3=  pd.DataFrame.from_dict(lol['EPS Trend'])
	analyst4=  pd.DataFrame.from_dict(lol['EPS Revisions'])
	analyst5=  pd.DataFrame.from_dict(lol['Growth Estimates'])
	
	return render_template("navbar2.html", user=user1, data=HTML(df.to_html(classes='table table-stripped')), charts=HTML(f.read()), charts2=HTML(g.read()), charts3=HTML(h.read()), balance=HTML(balance_sheet.to_html(classes='Table table-stripped')), analyst1=HTML(analyst1.to_html(classes='table table-stripped')), analyst2=HTML(analyst2.to_html(classes='table table-stripped')), analyst3=HTML(analyst3.to_html(classes='table table-stripped')), analyst4=HTML(analyst4.to_html(classes='table table-stripped')), analyst5=HTML(analyst5.to_html(classes='table table-stripped')), he1 = "Open, High, Close", he2 = "100 Day Moving Average vs. Close", he3 = "DGI Dashboard")
